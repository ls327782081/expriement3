...
评估指标：
- Stage 1: 重构损失、码本利用率、聚类质量

注意：Stage 2 (SASRec推荐训练) 已移至 train_sasrec_ahrq_ablation.py
"""
...
def save_results(all_results: List[Dict], output_dir: str = "./results/ahrq_ablation"):
    ...
    with open(readable_path, 'w', encoding='utf-8') as f:
        ...
        for result in all_results:
            ...
            f.write(f"  Recon Loss: {metrics['val_recon_loss']:.6f}\n")
            f.write(f"  Codebook Usage: {metrics['codebook_usage']['overall']:.4f}\n")
            # 打印各层使用率
            for i, rate in enumerate(metrics['codebook_usage']['by_layer']):
                f.write(f"  L{i} Usage: {rate:.4f}\n")
